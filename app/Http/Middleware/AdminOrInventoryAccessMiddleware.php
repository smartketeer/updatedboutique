...
class AdminOrInventoryAccessMiddleware
{
    /**
     * Allow the request if the user is an admin, or if they are a staff
     * member carrying a valid X-Inventory-Access-Token header.
     */
    public function handle(Request $request, Closure $next): Response
    {
        ...
        // Admins pass through without needing an inventory access token.
        if ($user->role === 'admin') {
            return $next($request);
        }

        // Staff must present a valid inventory access token.
